...
RUN dnf -y module enable ruby:${RUBY_VERSION} \
    && dnf install --setopt=skip_missing_names_on_install=False,tsflags=nodocs -y shared-mime-info make automake gcc gcc-c++ redhat-rpm-config postgresql git ruby-devel rubygem-irb rubygem-rdoc glibc-devel libpq-devel libyaml-devel xz \
    && dnf clean all \
    && rm -rf /var/cache/yum
